...
vmod_rule__fini(struct vmod_accept_rule **rulep)
{
	...
	TAKE_OBJ_NOTNULL(rule, rulep, RULE_MAGIC);
...
match_token(struct vmod_accept_rule *rule, VCL_STRING s, size_t l)
{
	...
	VTAILQ_FOREACH(t, &rule->tokens, list) {
		AN(t->string);
		if (l < t->length || strncasecmp(t->string, s, t->length))
			continue;
		if (l == t->length)
			break;
		if (s[t->length] == '-')
...
main(int argc, char *argv[])
{
	...
	f = fopen(argv[1] , "r");
	AN(f);
	is = fgets(buf, sizeof(buf), f);
	fclose (f);

	vmod_rule__init(NULL, &rule, NULL, NULL);
	CHECK_OBJ_NOTNULL(rule, RULE_MAGIC);
	...
	while (buf[i] && i < 100) {
		item[1] = '1' + (buf[i] % 4);
		if (buf[i] < 0) {
			vmod_rule_add(NULL, rule, item);
		} else {
			vmod_rule_remove(NULL, rule, item);
		}
		os = TRUST_ME(vmod_rule_filter(NULL, rule, is));
		AN(os);
		i++;
	}
	vmod_rule_add(NULL, rule, is);
	os = TRUST_ME(vmod_rule_filter(NULL, rule, is));
	vmod_rule_add(NULL, rule, is);
	os = TRUST_ME(vmod_rule_filter(NULL, rule, is));
	vmod_rule_remove(NULL, rule, is);
	vmod_rule_remove(NULL, rule, is);
	vmod_rule_remove(NULL, rule, is);
	vmod_rule__fini(&rule);
	return (0);
}
